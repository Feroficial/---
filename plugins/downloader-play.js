...
await conn.reply(m.chat, 'âŒ No se pudo descargar el audio', m || null)
}
...
await conn.reply(m.chat, 'âŒ No se pudo descargar el video', m || null)
}
}
} catch (error) {
console.error(error)
...
const apiUrl = `https://deliriussapi-oficial.vercel.app/download/ytmp4?url=${encodeURIComponent(yt_play[0].url)}`;
                ...
                if (!delius.status) return m.react("âŒ");
